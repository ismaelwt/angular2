...
import { Validation } from "../../../shared/models/validations";
...
export class EmpresaShowComponent implements OnInit {
  ...
  beforeSave() {
     ...
     let mensagens = new Array<Validation>();

    if (!this.empresa.nome) {
      let validation = new Validation('Favor inserir um nome', true);
      mensagens.push(validation);
    }
    if (!this.empresa.cnpj) {
      let validation = new Validation('Favor inserir um cnpj', true);
      mensagens.push(validation);
    }
    if (!this.empresa.cidade) {
      let validation = new Validation('Favor inserir uma cidade', true);
      mensagens.push(validation);
    }
    if (!this.empresa.endereco) {
      let validation = new Validation('Favor inserir um endereço', true);
      mensagens.push(validation);
    }
    if (!this.empresa.cep) {
      let validation = new Validation('Favor inserir um cep', true);
      mensagens.push(validation);
    }
    if (!this.empresa.bairro) {
      let validation = new Validation('Favor inserir um bairro', true);
      mensagens.push(validation);
    }
    if (!this.empresa.numero) {
      let validation = new Validation('Favor inserir um numero', true);
      mensagens.push(validation);
    }
    if (!this.empresa.ddd) {
      let validation = new Validation('Favor inserir um DDD', true);
      mensagens.push(validation);
    }
    if (!this.empresa.telefone) {
      let validation = new Validation('Favor inserir um Telefone', true);
      mensagens.push(validation);
    }
    if (!this.empresa.email) {
      let validation = new Validation('Favor inserir um email', true);
      mensagens.push(validation);
    }
    if (!this.empresa.complemento) {
      let validation = new Validation('Favor inserir um complemento', true);
      mensagens.push(validation);
    }
